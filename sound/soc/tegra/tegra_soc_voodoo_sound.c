...
void write_vol(struct snd_soc_codec *codec, unsigned int left, unsigned int right,unsigned short level)
{
	unsigned short val;

	val = level;
	val |= WM8903_HPOUTLZC;
	snd_soc_write(codec, left , val);

	val |= WM8903_HPOUTVU;
	snd_soc_write(codec, right, val);
}
static unsigned short parse_short(const char * buf)
{
...
{ write_vol(codec, WM8903_ANALOGUE_OUT1_LEFT, WM8903_ANALOGUE_OUT1_RIGHT, level); }

static ssize_t out2_write(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{ write_vol(global_codec, WM8903_ANALOGUE_OUT2_LEFT, WM8903_ANALOGUE_OUT2_RIGHT, parse_short(buf)); return size;}
static ssize_t out3_write(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{ write_vol(global_codec, WM8903_ANALOGUE_OUT3_LEFT, WM8903_ANALOGUE_OUT3_RIGHT, parse_short(buf)); return size;}
...
static ssize_t get_volume_masked_val(unsigned int reg, char * buf)
{
        unsigned short val;
        val = snd_soc_read(global_codec, reg);
        val &= ~(WM8903_HPOUTVU_MASK);
        val &= ~(WM8903_HPOUTLZC_MASK);
        val &= ~(WM8903_HPL_MUTE_MASK);
...
static ssize_t out2_amplifier_level_show(struct device *dev, struct device_attribute *attr, char *buf)
{ return get_volume_masked_val(WM8903_ANALOGUE_OUT2_LEFT, buf); }

static ssize_t out3_amplifier_level_show(struct device *dev, struct device_attribute *attr, char *buf)
{ return get_volume_masked_val(WM8903_ANALOGUE_OUT3_LEFT, buf); }
				     ...
				     struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "WM%X\n%u\n",
		       ...
		       snd_soc_read(global_codec, WWM8903_CP_ENA)
	);
}
...
static DEVICE_ATTR(out2_amplifier_level, S_IRUGO  | S_IWUGO, out2_amplifier_level_show, out2_write);
static DEVICE_ATTR(out3_amplifier_level, S_IRUGO | S_IWUGO, out3_amplifier_level_show, out3_write);
...
static struct attribute *voodoo_sound_attributes[] = {
	...
	&dev_attr_out2_amplifier_level.attr,
	&dev_attr_out3_amplifier_level.attr,
